"""Preentrenamiento no supervisado (autoencoder) por fold, comun a los 3
metodos. Se entrena UN SOLO autoencoder de farmaco y UNO de proteina por
fold, usando solo los descriptores (sin la afinidad Y) de las entidades
UNICAS que aparecen en el train de ese fold -- nunca ve las del fold de
test, asi que no viola la regla de cold-drug ni la de "todo se ajusta solo
con train".

El embedding resultante (128 dims) se concatena a los descriptores
seleccionados/escalados antes de entrenar el modelo supervisado de cada
metodo -- el modelo recibe una entrada mas grande, pero cada metodo sigue
sin saber nada de autoencoders: esto vive enteramente en common/.
"""
